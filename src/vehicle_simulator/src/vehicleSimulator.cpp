...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubVehicleOdomAtImage = nh.advertise<nav_msgs::Odometry> ("/state_estimation_at_image", 5);
  ...
  tf::StampedTransform odomTrans, odomTransAtImage;
  ...
  while (status) {
    ...
    if (newRgbImage && newDepthImage && fabs(rgbImageTime - depthImageTime) < 0.005 && rgbImageTime > stateAtImageTime || !stateInit) {
      ...
      pubVehicleOdomAtImage.publish(odomData);
      
      odomTransAtImage = odomTrans;
      odomTransAtImage.child_frame_id_ = "vehicle_at_image";
      
      stateAtImageTime = timeNow.toSec();
      stateInit = true;
    }
    
    odomTransAtImage.stamp_ = timeNow;
    tfBroadcaster.sendTransform(odomTransAtImage);
